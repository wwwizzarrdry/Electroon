...
//var config = roon.load_config("config") || {};

var core;
var roon = new RoonApi({
    extension_id:        'com.wwwizzarrdry.electroon',
    display_name:        'Roon Mini Controller',
...
var v = new Vue({
    ...
    data: function() { 
        return {
            ...
            server_port: settings.server_port || 9100,
            status: 'null',
            zones: [],
    ...
    computed: {
        ...
        artist_img: function() {
            this.artist_img = 'http://' + this.server_ip + ':' + this.server_port + '/api/image/' + this.zones[this.current_zone_id].now_playing.artist_image_keys[0] + '?scale=fit&width=1920&height=1080';
        ...
        album_img: function() {
            this.album_img = 'http://' + this.server_ip + ':' + this.server_port + '/api/image/' + this.zones[this.current_zone_id].now_playing.image_key + '?scale=fit&width=1920&height=1080';
            return 'http://' + this.server_ip + ':' + this.server_port + '/api/image/' + this.zones[this.current_zone_id].now_playing.image_key + '?scale=fit&width=1920&height=1080';
        }
    },
    watch: {
        ...
        'zones[current_zone_id].now_playing.image_key': function(img) {
            var ts = new Date;
            ipcRenderer.send('nowPlaying', {
                "artist_img": this.artist_img,
                "album_img": this.album_img,
                "zone": this.zone
            });
            this.history.push({
                timestamp: ts.valueOf(),
                profile: this.zones[this.current_zone_id].settings.profile,
                line1: this.zones[this.current_zone_id].now_playing.three_line.line1,
                line1: this.zones[this.current_zone_id].now_playing.three_line.line1,
                line1: this.zones[this.current_zone_id].now_playing.three_line.line1,
                artist_img: this.artist_img,
                album_img: this.album_img
            });
            roon.save_config("history", history);
        }
    },
    methods: {
        to_time: function (s) {
